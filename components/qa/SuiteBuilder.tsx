...
export function SuiteBuilder({ agents, selectedAgent, agentUrl, onAgentChange }: Props) {
  ...
  const durationError =
    durationValue && isNaN(Number(durationValue)) ? "Duration must be a number." : null;
  const canAddCase =
    !metadataError && !regexError && !jsonPathError && !dataTableError && !durationError;
  ...
  const handleImportFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = e => {
      ...
      try {
        ...
        const validated = imported.map((c): QaTestCase => ({
  ...
  return (
    <div className="bg-card flex min-w-0 flex-col gap-4 rounded-lg border p-5 shadow-xs">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <ClipboardCheckIcon className="text-fg-subtle size-4" />
      ...
      <div className="space-y-2">
        ...
        <Select value={selectedAgent?.url ?? agentUrl} onValueChange={onAgentChange}>
          <SelectTrigger aria-label="Select target agent">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {agents.map(agent => (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-1">
            {draftCases.map((c, i) => (
              <div
                key={c.id}
                className="bg-surface-2 flex items-center justify-between rounded-md px-2.5 py-1.75 text-sm"
              >
      ...
      <div className="border-t pt-3">
        <Small className="mb-3 block">{draftCases.length > 0 ? "Add another case" : "Case"}</Small>
        <div className="flex flex-col gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select value={expectedTaskState} onValueChange={setExpectedTaskState}>
                <SelectTrigger aria-label="Expected task state">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={v => setExpectedOutputMode(v as QaOutputMode)}
              >
                <SelectTrigger aria-label="Expected output mode">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="border-t pt-2">
            <Small className="mb-2 block">Assertions</Small>
            <div className="flex flex-col gap-2">
              <div className="space-y-1.5">
                <Label className="text-muted-foreground text-xs">Regex pattern</Label>
                <Input
                  placeholder="e.g. ready|ok"
                  value={regexPattern}
              ...
              <div className="space-y-1.5">
                <Label className="text-muted-foreground text-xs">JSON path exists</Label>
                <Input
                  placeholder="$.status"
                  value={jsonPath}
              ...
              <div className="space-y-1.5">
                <Label className="text-muted-foreground text-xs">Task duration (ms)</Label>
                <div className="flex gap-2">
                  <Select
                    value={durationOperator}
                    onValueChange={v => setDurationOperator(v as typeof durationOperator)}
                  >
                    <SelectTrigger className="w-32 shrink-0" aria-label="Duration operator">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-1.5">
                <Label className="text-muted-foreground text-xs">Artifact MIME pattern</Label>
                <Input
                  placeholder="image/* or application/json"
                  value={artifactMimePattern}
                  onChange={e => setArtifactMimePattern(e.target.value)}
                />
              </div>
            </div>
          </div>
      ...
      <Caption className="text-muted-foreground">
        Import accepts <strong>.json</strong> (array of case objects) or <strong>.csv</strong>{" "}
        (columns: name, prompt, expectedTaskState, expectedOutputMode, regexPattern, jsonPath,
        metadata).
      </Caption>
    </div>
  );
